...
> - **已读卡片过期清理**：`retentionDays`（默认 3 天）配置已读卡片保留天数，每次采集（loadAll）前清除过期已读卡片（按 `readAt`，旧数据回退 `createdAt`）；未读卡片不按天清除；
> - 以下 v1 章节中与多源、兴趣词、规则学习、屏蔽日志、不感兴趣反馈相关的描述已不再适用，仅作历史参考。

## 1. 背景与目标

用户在 DSH Web GUI 中希望有一块「右侧实时讯息区」：周期性（默认 60 分钟）从若干**可配置的搜索源**采集内容，由大模型判断重要性并生成摘要，以**卡片**形式展示在右侧区域；点击卡片打开原文链接。搜索源必须易于追加，因此所有源共用一套**基类模板**，源脚本只需实现「粗搜（返回标题等粗略信息）」与「精搜（输入粗搜结果、返回详细内容）」两个方法。
