 ...
 * 约定：
 * - 仅可用 Builtins（动态模式）：ctx / harness / console / btoa / atob / TextEncoder / TextDecoder；
...
return {
  ...
  apply(ctx) {
    ...
    const SUMMARIZE_BATCH = 4; // 每次模型调用处理的精读条目数（控制单次输入规模）
    ...
    // 批量精读+摘要：抓取仍逐簇进行；模型摘要调用按批次合并（一批一次调用返回全部结果）
    async function stageFineAndSummarizeAll(clusters) {
      ...
      for (let start = 0; start < entries.length; start += SUMMARIZE_BATCH) {
        const batch = entries.slice(start, start + SUMMARIZE_BATCH);
        const results = new Array(batch.length).fill(null);
        try {
          const system =
            '你是资讯摘要助手。对每条内容生成「' + lang + '」标题与 2-3 句摘要。标题必须翻译成「' + lang +
            '」（专有名词/产品名/品牌名可保留原文）。只输出 JSON 数组，顺序对应输入，' +
            '每条都必须包含 index/title/summary：[{"index":0,"title":"…","summary":"…"}]';
          const input = JSON.stringify(batch.map((e, i) => ({ index: i, title: e.main.item.title, content: e.sourceText })));
          const raw = await callModel(system, input, 5000);
          const parsed = extractJsonArr(raw);
          if (Array.isArray(parsed)) {
            for (const p of parsed) {
              if (!p || typeof p.index !== 'number' || !Number.isInteger(p.index) || p.index < 0 || p.index >= batch.length) continue;
              if (p.title && p.summary) results[p.index] = { title: String(p.title), summary: String(p.summary) };
            }
          }
        } catch (err) {
          console.error('[dsh-livefeed] batch summarize failed:', String((err && err.message) || err));
        }
