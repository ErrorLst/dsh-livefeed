...
return {
  ...
  apply(ctx) {
    ...
    async function fetchViaCurl(url) {
      ...
      // （bash-sandbox 对 danger-full-access 直接放行、不套沙箱），不改动部署默认策略。
      // PATH 提示：本机 dsh web 进程的 PATH 不含 git bash 目录，bash/curl 可能解析失败；
      // 前置常见 git usr/bin 目录，让 bash 解析到真正的 msys2 bash（找不到时按原 PATH 回退）。
      const origPath = (typeof process !== 'undefined' && process.env && process.env.PATH) || '';
      const gitHints = ['C:\\Program Files\\Git\\usr\\bin', 'C:\\Program Files\\Git\\bin', 'C:\\Program Files (x86)\\Git\\usr\\bin'];
      const spec = shell.resolve({
        command: 'curl -sL --max-time 20 --compressed "' + safeUrl + '"',
        timeoutMs: 30000,
        stdoutMaxBytes: 1200000,
        env: { PATH: gitHints.join(';') + (origPath ? ';' + origPath : '') },
        sandboxPolicy: {
    ...
    async function stageJudge(source, items) {
      ...
      for (let i = cap; i < out.length; i++) pushFilterLog(out[i], source, 'max-candidates');
