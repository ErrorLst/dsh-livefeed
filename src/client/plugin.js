/* ═══════════════════════════════════════════════════════════════════════════
 * dsh-livefeed Client 半 —— cordis_define(code.client) 的函数体（完整实现 v1）
 * ═══════════════════════════════════════════════════════════════════════════
 * 约定：
 * - 本文件内容整体作为 code.client 传入 cordis_define（不含外层 function 声明）；
 * - 仅可用 Builtins：ctx / React / host / styles / console；React 用 createElement，禁止 JSX；
 * - 不使用 window/document 全局（Guard 未声明）；坐标类取元素自身 DOM API；
 * - 颜色一律 var(--dsw-alias-*) / var(--dsw-static-*)，自动适配明暗主题。
 */
return {
  inject: ['timer'],
  apply(ctx) {
...
`;
    ...
    function CardItem(props) {
      ...
      const cls = 'lf-card' + (card.read ? ' lf-read' : '') + (isDislike ? ' lf-dislike' : '');
      return h('div', { className: 'lf-card-wrap' },
        ...
        h('a', {
          ...
          card.summary ? h('div', { className: 'lf-card-summary' }, card.summary) : null,
    ...
    function LiveFeedPanel() {
      ...
      const refresh = React.useCallback(async () => {
        try {
          const res = await host.call('dsh-livefeed/cards');
      ...
      async function call(method, args) {
        try { return await host.call(method, args || {}); } catch (e) { console.error('[dsh-livefeed] rpc failed', method, e); return null; }
      }
      ...
      const groups = [
        { key: 'latest', label: '最新', items: cards.filter((c) => !c.read && c.isNew) },
        { key: 'unread', label: '未读', items: cards.filter((c) => !c.read && !c.isNew) },
        { key: 'read', label: '已读', items: cards.filter((c) => c.read && c.feedback !== 'dislike') },
        { key: 'dislike', label: '不感兴趣', items: cards.filter((c) => c.feedback === 'dislike') },
      ...
      const listContent = groups.length
        ? groups.map((g) =>
            h('div', { key: g.key },
              h('div', { className: 'lf-group' + (collapsedGroups[g.key] ? ' lf-collapsed' : ''), onClick: () => toggleGroup(g.key) },
                h('span', { className: 'lf-group-chev' }, iconSvg('m6 9 6 6 6-6', 12)),
                h('span', { className: 'lf-group-label' }, g.label),
                h('span', { className: 'lf-group-count' }, String(g.items.length)),
              ),
              collapsedGroups[g.key] ? null : g.items.map((c) =>
                h(CardItem, {
                  key: c.id, card: c,
        ...
        : h('div', { className: 'lf-empty' },
            h('div', { className: 'lf-empty-icon' }, iconSvg('M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3z', 18)),
            h('p', null, '暂无内容'),
            h('p', { className: 'sub' }, status.paused ? '采集已暂停' : '在设置中启用搜索源后开始采集'),
            h('button', { className: 'lf-ghostbtn', onClick: () => { call('dsh-livefeed/refresh'); showToast('已触发刷新'); } }, '立即刷新'),
          );
    ...
    function SettingsView(props) {
      ...
      const [intervalMin, setIntervalMin] = React.useState(10);
      ...
      React.useEffect(() => {
        (async () => {
          const c = await host.call('dsh-livefeed/config');
          if (c && c.config) {
            setCfg(c.config);
            setIntervalMin(c.config.intervalMinutes || 10);
          ...
          const cat = await host.call('dsh-livefeed/model-catalog');
          if (cat && Array.isArray(cat.providers)) setCatalog(cat.providers);
          const r = await host.call('dsh-livefeed/rules');
          if (r && r.rules) setRules(r.rules);
          const fl = await host.call('dsh-livefeed/filter-log');
      ...
      async function saveAll() {
        ...
        await host.call('dsh-livefeed/update-settings', payload);
        if (wordDirty) await host.call('dsh-livefeed/update-words', { interests, blockWords });
        ...
        h('div', { className: 'lf-scroll' },
          ...
          h('div', { className: 'lf-sec' },
            ...
            h('label', { className: 'lf-field' }, h('span', null, '思考等级（reasoning effort）'), h('select', {
              value: effort, disabled: useDefault,
              onChange: (e) => setEffort(e.target.value),
            }, EFFORTS.map((x) => h('option', { key: x.id, value: x.id }, x.name)))),
          ...
          h('div', { className: 'lf-sec' },
            ...
            h('div', { style: { display: 'flex', gap: 8, marginTop: 10 } },
              h('button', { className: 'lf-ghostbtn', onClick: async () => { await host.call('dsh-livefeed/rerun-rules'); props.showToast('规则重训已触发'); } }, '立即重新学习'),
            ),
          ),
          // 兴趣词
          h('div', { className: 'lf-sec' },
          ...
          // 被屏蔽内容
          h('div', { className: 'lf-sec' },
            h('div', { className: 'lf-sec-title' }, '被屏蔽内容（可撤销）'),
            filterLog.length ? filterLog.map((it, i) => h('div', { key: i, className: 'lf-filter-item' },
              h('div', { className: 'lf-filter-info' },
                h('div', { className: 'lf-filter-title' }, it.title),
                h('div', { className: 'lf-filter-meta' }, it.sourceId + ' · ' + fmtTime(it.ts ? Date.parse(it.ts) : 0) + ' ',
                  h('span', { className: 'lf-filter-reason' }, it.reason === 'block-keyword' ? '屏蔽词命中' : '模型筛选')),
              ),
              h('button', { className: 'lf-unblock', onClick: async () => {
                await host.call('dsh-livefeed/unblock', { url: it.url });
                props.showToast('已撤销：该条目已恢复为未读卡片 ✓');
                props.refresh();
                const fl = await host.call('dsh-livefeed/filter-log');
                if (fl && Array.isArray(fl.items)) setFilterLog(fl.items);
              } }, '撤销'),
            )) : h('p', { className: 'lf-rules-meta' }, '暂无被屏蔽内容'),
          ),
          // 数据归档
          h('div', { className: 'lf-sec' },
          ...
          h('div', { className: 'lf-save-row' },
            ...
            h('button', { className: 'lf-ghostbtn', onClick: () => {
              setUseDefault(true); setIntervalMin(10); setArchiveMax(5000);
